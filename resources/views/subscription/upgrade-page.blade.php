...
<div class="container-fluid py-1">
    ...
    @if($statusAssinatura['status'] !== 'ativa')
        <div class="alert alert-{{ $statusAssinatura['tipo_alerta'] }} border-0 shadow-sm mb-5 p-4 container-fluid" role="alert">
            <div class="d-flex align-items-center">
                <i class="fas fa-exclamation-triangle fa-2x me-3"></i>
                <div>
                    <h5 class="mb-1 fw-bold">{{ $statusAssinatura['titulo'] }}</h5>
                    <p class="mb-0">
                        {{ $statusAssinatura['mensagem'] }}
                        @if($statusAssinatura['acao_sugerida'])
                            <strong>{{ $statusAssinatura['acao_sugerida'] }}</strong>
                        @endif
                    </p>
                </div>
            </div>
        ...
        <div class="row mb-4">
            @foreach($alertasAtivos as $alerta)
                <div class="col-12 mb-3">
                    <div class="alert alert-{{ $alerta->getBadgeClass() }} border-0 shadow-sm p-3" role="alert">
                        <div class="d-flex align-items-center">
                            <i class="fas fa-{{ $alerta->getTipoIcone() }} fa-lg me-3"></i>
                            <div class="flex-grow-1">
                                <h6 class="mb-1 fw-bold">{{ $alerta->titulo }}</h6>
                                <p class="mb-0 small">{{ $alerta->mensagem }}</p>
                                <small class="text-muted">{{ $alerta->getCriadoEmRelativo() }}</small>
                            </div>
                            <button class="btn btn-sm btn-outline-secondary ms-3"
                                    wire:click="marcarAlertaLido({{ $alerta->id }})">
...
<div class="container-fluid">
    ...
    <div class="pricing-header text-center position-relative">
        <h2 class="fw-bold mb-3 text-info display-5">
            ...
            @else
                <i class="fas fa-arrow-up me-3"></i>{{ $modoFormatado }}
            @endif
        </h2>
    ...
    <livewire:subscription.pacote-cards
        ...
        :pacote-atual="$pacoteAtual"
        :assinatura-atual="$assinaturaAtual"
        key="pacote-cards"
    />
    ...
    <div id="comparisonTable"
         style="{{ $showComparison ? '' : 'display: none;' }}">

            <div class="container-fluid py-5">
                @include('subscription.partials.comparison-table', ['pacotes' => $pacotesDisponiveis])
            </div>
    ...
    @if($historicoAssinaturas->count() > 0)
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @foreach($historicoAssinaturas as $assinatura)
                                <tr>
                                    <td>
                                        <strong class="text-secondary">{{ Str::ucfirst($assinatura->pacote->nome) }}</strong>
                                    </td>
                                    <td>
                                        <b class="text-danger">{{ $assinatura->data_inicio->format('d/m/Y') }}</b> -
                                        <b class="text-warning">{{ $assinatura->data_fim->format('d/m/Y') }}</b>
                                    </td>
                                    <td class="text-success fw-bold">
                                        {{ $assinatura->getValorFormatado() }}
                                    </td>
                                    <td>
                                        <span class="badge bg-{{ $assinatura->status === 'Ativo' ? 'success' : 'secondary' }}">
                                            {{ $assinatura->status }}
                                        </span>
                                    </td>
                                </tr>
    ...
    <div class="card bg-info text-light text-white border-0 shadow-lg mt-5">
        <div class="card-body p-5 text-center">
            ...
            <div class="d-flex justify-content-center gap-3 flex-wrap mt-4">
                ...
                @guest
                <a href="{{ route('ecommerce.trial.solicitar') }}" class="btn btn-outline-light btn-lg px-4 fw-bold shadow-sm">
